...
class Post extends React.Component {
  constructor(props) {
    ...
    this.state = {
      userName: "user_name",
      selfComment: "Lorem ipsum pricsndf eioask dfjejo dklf djiwoeovmoc dihevo",
      comments: [
  ...
  render() {
    let show_button = (
      <p className="post__view-button" onClick={()=>{this.setState({commentsOpened: true})}}>
        View all {this.state.comments.length.toString()} comments
      </p>
    );
    let hide_button = <p className="post__view-button" onClick={()=>{this.setState({commentsOpened: false})}}>Hide comments</p>;
    ...
    return (
      <div className="post">
        ...
        <div className="post__img">
          <img src="https://picsum.photos/512"></img>
        </div>
        <div className="post__after-img-zone">
          ...
          <div className="post__comment">
            ...
            &nbsp;
            <span>{this.state.selfComment}</span>
          </div>
          {comment_button}
          {comments.map((item, index) => {
            return <p className="post__comment" key={index}><span className="post__comment_user-name">{item[0]}</span> &nbsp;{item[1]}</p>
          })}
        </div>
      </div>
    );
